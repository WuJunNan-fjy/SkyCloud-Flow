...
export default {
  ...
  methods: {
    ...
    /* 高亮流程图 */
    // TODO wjn：如果多个 endActivity 的话，目前的逻辑可能有一定的问题。https://www.jdon.com/workflow/multi-events.html
      ...
      this.bpmnModeler.getDefinitions().rootElements[0].flowElements?.forEach(n => {
        ...
        if (n.$type === 'bpmn:UserTask') { // 用户任务
          ...
          outgoing?.forEach(nn => {
            ...
            } else if (nn.targetRef.$type === 'bpmn:ExclusiveGateway') { // TODO wjn：这个流程，暂时没走到过
            ...
            } else if (nn.targetRef.$type === 'bpmn:EndEvent') { // TODO wjn：这个流程，暂时没走到过
