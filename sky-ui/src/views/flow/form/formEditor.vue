            ...
            <el-form ref="form" :model="form" :rules="rules" label-width="80px">
              ...
              <el-form-item label="开启状态" prop="status">
                <el-radio-group v-model="form.status">
                  <el-radio v-for="dict in dict.type.common_status" :key="dict.value" :label="dict.value">{{ dict.label }}</el-radio>
                </el-radio-group>
              </el-form-item>
...
import RightPanel from '@/views/tool/build/RightPanel.vue'
import FormDrawer from '@/views/tool/build/FormDrawer.vue'
import JsonDrawer from '@/views/tool/build/JsonDrawer.vue'
import CodeTypeDialog from '@/views/tool/build/CodeTypeDialog.vue'
import drawingDefalut from '@/components/generator/drawingDefalut'
import DraggableItem from '@/views/tool/build/DraggableItem.vue'
import {
...
import {createForm, getForm, updateForm} from "@/api/flow/form";
...
export default {
  ...
  created() {
    ...
    if (formId) {
      getForm(formId).then(response => {
        ...
        this.drawingList = decodeFields(data.fieldList)
        // 设置激活的表单项
  ...
  methods: {
    ...
    save() {
      this.$refs["form"].validate(valid => {
        ...
        const form = {
          ...
          fieldList: this.encodeFields(), // 表单项的数组
          ...this.form // 表单名等
        }
        // 修改的提交
    ...
    close() {
      // 然后关闭页面
      this.$tab.closeOpenPage({ path: "/flow/form/index" });
    },
    encodeFields() {
      const fields = []
